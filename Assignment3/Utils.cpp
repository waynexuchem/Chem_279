...
arma::vec calculateCenter(arma::vec centerA, arma::vec centerB, double alpha, double beta) {
    return (alpha * centerA + beta * centerB) / (alpha + beta);
}
...
double calculateOverlapIntegral_1D(int dimension, arma::vec centerA, arma::vec centerB, double alpha, double beta, int lA, int lB) {
    ...
    // Calculate the first component of the overlap integral
    double component_1 = exp(-alpha * beta * pow(centerA[dimension] - centerB[dimension], 2) / (alpha + beta));

    // Calculate the second component of the overlap integral
    double component_2 = sqrt(M_PI / (alpha + beta));

    // Calculate the third component of the overlap integral
    double component_3 = 0.0;
    
    for (int i = 0; i <= lA; i++) {
        for (int j = 0; j <= lB; j++) {
            if ((i + j) % 2 == 0) {
                component_3 += bionomial_coefficient(lA, i) * bionomial_coefficient(lB, j) * \
                ((doubleFactorial(i + j - 1) * pow(centerP(dimension) - centerA(dimension), lA - i) \
                * pow(centerP(dimension) - centerB(dimension), lB - j)) / pow(2 * (alpha + beta), double(i + j)/2));
            }
        }
    }
...
double calculateOverlapIntegral_ss(Shell ShellA, Shell ShellB) {
    ...
    result = calculateOverlapIntegral_1D(0, centerA, centerB, alpha, beta, lA, lB) * \
            calculateOverlapIntegral_1D(1, centerA, centerB, alpha, beta, lA, lB) * \
            calculateOverlapIntegral_1D(2, centerA, centerB, alpha, beta, lA, lB);
...
std::vector<double> calculateOverlapIntegral_sp(Shell sShell, Shell pShell) {
    ...
    for (int l = 0; l < 3; ++l) {
        ...
        overlap_integral = calculateOverlapIntegral_1D(0, centerA, centerB, alpha, beta, lA, lB[0]) * \
                calculateOverlapIntegral_1D(1, centerA, centerB, alpha, beta, lA, lB[1]) * \
                calculateOverlapIntegral_1D(2, centerA, centerB, alpha, beta, lA, lB[2]);
        result[l] = overlap_integral;
    }
    return result;
}
...
arma::mat calculateOverlapIntegral_pp(Shell ShellA, Shell ShellB) {
    ...
    for (int l=0; l<3; l++){
        ...
        for (int m=0; m<3; m++){
            ...
            overlap_integral = calculateOverlapIntegral_1D(0, centerA, centerB, alpha, beta, lA[0], lB[0]) * \
                calculateOverlapIntegral_1D(1, centerA, centerB, alpha, beta, lA[1], lB[1]) * \
                calculateOverlapIntegral_1D(2, centerA, centerB, alpha, beta, lA[2], lB[2]);
            result(l, m) = overlap_integral;
            }
        }
    return result;
}
